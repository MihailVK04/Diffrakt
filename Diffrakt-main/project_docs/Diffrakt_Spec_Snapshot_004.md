# Diffrakt — Project Spec Snapshot 004
...
> **Changed from Snapshot 003:** The SPA shell is served by a static `public/index.html`; `index.php` is a pure API front controller that only ever handles `/api/v1/*` requests and returns JSON. `.htaccess` and the Nginx config are updated to route `/api/*` to `index.php` and everything else to `index.html`. `home.js` is added to the views — it is the landing page shown to unauthenticated visitors. The client-side route table is fully defined. `app.js` is built — it imports `api.js` as an ES module, owns the regex router, guards auth-required routes, manages the active view lifecycle, and exposes `app.navigate()` and `app.refreshUser()` globally. All other architecture is unchanged.
