# Diffrakt — Project Spec Snapshot 003
...
> **Changed from Snapshot 002:** `rate_limits` table schema is now fully defined. `RateLimiter.php` is built — it uses a fixed-window counter, hashes the client IP with SHA-256, and performs a single atomic upsert per request. `Validator.php` is built — supports `required`, `email`, `min_length`, `max_length`, `integer`, `min`, `max`. `api.js` is updated — token store removed, `credentials: 'include'` added to every fetch call. All other architecture is unchanged.
