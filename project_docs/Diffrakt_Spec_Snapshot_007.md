# Diffrakt — Project Spec Snapshot 007
...
> **Changed from Snapshot 006:**
> - `POST /api/v1/pipelines/{id}/apply` is now strictly a post-based export endpoint — it accepts `post_id` and saves the result to `storage/processed/`.
> - `POST /api/v1/pipelines/{id}/preview` is a new stateless endpoint — it accepts `image_b64`, runs the pipeline on a temporary file, and returns `image_b64`. No file is persisted. Used by the editor for live server-side preview.
> - `api.js` — `pipelines.apply()` renamed to `applyToPost(postId)`. New `pipelines.preview(imageB64, signal)` method added.
> - `api.js` — `pipelines.replaceSteps()` now correctly wraps the array as `{ steps: [...] }`.
> - `sessions` table — `user_id` column is now `INT NULL` (was `NOT NULL`). Required because `Session::write()` fires before `$_SESSION['user_id']` is set on the first unauthenticated request.
> - `Session::write()` — `expires_at` is now computed in SQL via `DATE_ADD(NOW(), INTERVAL ? SECOND)` instead of PHP `date()`, eliminating timezone mismatch between PHP and MySQL.
> - `Bootstrap.php` — `Database::getInstance()->getPdo()` is now passed to `RateLimiter` (was incorrectly passing the `Database` instance directly).
> - `PipelineController::replaceSteps()` — ownership check added before step replacement.
> - `Request::body()` — result is now cached in `$bodyCache` to prevent double-reads of `php://input`.
> - `Request::ip()` — `X-Forwarded-For` now only trusted when `APP_ENV=production`, matching `RateLimiter` behaviour.
> - `Validator` — `checkMin()`/`checkMax()` error messages now use `$label` instead of `$value` (was an XSS vector). Unknown rules now throw `\InvalidArgumentException` instead of silently passing. `label()` uses `ucwords` instead of `ucfirst`.
> - `UserController` — `followed_id` column references corrected to `followee_id` in all three follow queries (profile, follow, unfollow).
...
`expires_at` is computed in SQL via `DATE_ADD(NOW(), INTERVAL ? SECOND)` — never in PHP — to avoid timezone mismatch between PHP and MySQL.
...
`X-Forwarded-For` is only read when `APP_ENV=production`. In local development `REMOTE_ADDR` is always used, preventing clients from spoofing their IP by sending the header directly. This applies both in `RateLimiter` and `Request::ip()`.
...
| Method | Endpoint | Auth | Notes |
|--------|----------|------|-------|
| GET | `/pipelines/{id}` | No | Pipeline with ordered steps — sub-pipelines are recursively flattened into atomic steps before the response is sent |
...
| POST | `/pipelines/{id}/apply` | Yes | Applies pipeline to an existing post: accepts `post_id`, saves result to `storage/processed/`, returns `processed_path` |
| POST | `/pipelines/{id}/preview` | Yes | Stateless preview: accepts `image_b64`, runs pipeline on a temporary file, returns `image_b64`. Nothing is persisted. |
...
### Sub-pipeline flattening (Option B)

`GET /pipelines/{id}` resolves all `sub_pipeline_id` references recursively and replaces them with the constituent atomic steps inline before returning the response. The client always receives a flat list of atomic steps — `filter_id` is set, `sub_pipeline_id` is always `null`. This allows `pipeline.js` to run live preview entirely client-side without any server round-trips per preview frame.

The flattening is performed by `PipelineController` (not `PipelineRunner`) since it is a read-time concern, not an execution concern. The hard depth limit of 5 levels enforced by `PipelineRunner` still applies at write time (when steps are saved via `PUT /pipelines/{id}/steps`).

The stored data in `pipeline_steps` is unchanged — `sub_pipeline_id` rows are preserved as-is. Flattening is only applied to the API response.
...
| Use case | Final export button | Editor live preview |
...
- **Session handler** — implements `SessionHandlerInterface`, reads/writes the `sessions` MySQL table via PDO. Configured before `session_start()` in `Session::start()`. `expires_at` computed in SQL.
- **Router** — reads `REQUEST_URI`, strips query string, matches regex patterns with named segments like `{id}`. Returns after controller dispatch to prevent loop fallthrough.
- **Validator** — rules array (`required`, `email`, `min_length`, `max_length`, `integer`, `min`, `max`), returns per-field errors. One error per field; non-required rules skip empty values. Unknown rules throw `\InvalidArgumentException`.
- **Rate limiter** — SHA-256 hashes client IP, upserts `rate_limits` table with a single atomic query, calls `Response::tooManyRequests()` on breach. `X-Forwarded-For` only trusted in production.
