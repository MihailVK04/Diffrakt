# Diffrakt — Project Spec Snapshot 006
...
> **Changed from Snapshot 005:** `canvas.js` and `pipeline.js` are added to `public/assets/js/filters/`. `canvas.js` implements all ten atomic filters (IDs 1–10) using the Canvas API. `pipeline.js` exposes `runPipeline(source, steps)` and `runPipelineToCanvas(source, steps)` — the client-side runner that chains canvas filters for live preview. Sub-pipeline flattening (Option B) is adopted: `GET /pipelines/{id}` now returns steps with all `sub_pipeline_id` references recursively resolved and inlined as atomic steps before the response is sent. The client runner therefore only ever sees atomic filter steps. `PipelineController` is updated to flatten steps before returning them. All other architecture is unchanged.
...
│   │   ├── PipelineController.php  # CRUD + apply (server-side preview) + response-time step flattening
